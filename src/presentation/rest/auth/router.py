    ...
    *,
    ...
    try:
        response = await login_password_use_case.execute(
            phone_number=login_password_dto.username, password=login_password_dto.password
        )
        return response
        ...
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=str(credential_error))
